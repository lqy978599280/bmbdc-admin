<template>
  <div>
    <div class="main-top">
      ...
      <pictureEdit :data="houseResData[index]" :dialogFormVisible="dialoginf"
               @getdialogfv="getdialogfv" :title="title[2]"
               :readOnly='true' :pass_id="select_id" @F5="handleCurrentChange"></pictureEdit>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleCurrentChange() {
                const axios = require("axios")
                axios.get(`${this.global.config.url}/admin/wholesceneOrders/selectAllWholesceneOrdersList?page=${this.currentPage}&size=8`)
                    .then((response) => {
                        console.log(response);
                        this.houseResData = response.data.data.list;
