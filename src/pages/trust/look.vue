<template>
  <div>
    <div class="main-top">
      ...
      <lookEdit :data="overallViewData[index]"
               :dialogFormVisible="dialoginf"
               @getdialogfv="getdialogfv"
               :readOnly='true'
               :pass_id="select_id"
               :title="title"
               @F5="handleCurrentChange"></lookEdit>
...
import lookEdit from "./lookEdit";
    export default {
        ...
        watch:{
            'search':function () {
                this.delay.delay(()=>{
                    ...
                    axios.get(`${this.global.config.url}/admin/dkfOrders/selectAllDkfOrdersByValue?value=${this.search}&page=${this.currentPage}&size=8`)
                        .then((response) => {
        ...
        methods: {
            ...
            handleCurrentChange() {
                const axios = require("axios")
                axios.get(`${this.global.config.url}/admin/dkfOrders/selectAllDkfOrders?page=${this.currentPage}&size=8`)
                    .then((response) => {
                        console.log(response);
                        this.overallViewData = response.data.data.list;
                        this.total = response.data.data.total
