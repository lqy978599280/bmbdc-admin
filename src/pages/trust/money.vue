<template>
  <div>
    <div class="main-top">
      ...
      <moneyEdit :data="overallViewData[index]"
               ...
               @F5="handleCurrentChange"></moneyEdit>
...
import moneyEdit from "./moneyEdit";
    export default {
        ...
        methods: {
                            ...
                            this.overallViewData[i].status == 2 ? '待买方支付' :
                                this.overallViewData[i].status == 3 ? '待房产经纪人接单' :
                                    this.overallViewData[i].status == 4 ? '待买卖双方签署合同' :
                                        this.overallViewData[i].status == 5 ? '待财务确认':
            ...
            handleCurrentChange() {
                const axios = require("axios")
                axios.get(`${this.global.config.url}/admin/djcontractOrders/selectAllDjcontractOrders?page=${this.currentPage}&size=8`)
                    .then((response) => {
                        console.log(response);
                        this.overallViewData = response.data.data.list;
                        this.total = response.data.data.total
